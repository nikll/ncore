...
function rmdir_recursive($dir) {
	...
	foreach (scandir($dir) as $elem) if ($elem != '.' && $elem != '..') {
		if (is_dir($dir.'/'.$elem))
			rmdir_recursive($dir.'/'.$elem);
		else	unlink($dir.'/'.$elem);
	}
...
function meta_generator($text, $count = 25, $min_length = 4, $ignore = [], $as_array = false) {
	...
	$ret = [];
	foreach ($text as $sl) if (mb_strlen($sl) >= $min_length) { // собираем в массив тока слова не меньше указанной длины
...
function php_packer(array $files, $output_file_name) {
	...
	foreach ($files as $file) {
		...
		foreach ($tokens as $token) {
			if (is_array($token)) $new_code[] = $token;
